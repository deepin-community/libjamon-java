...
final class HttpMonRequest implements HttpMon {
    ...
    private String[] details=new String[]{null,""};// URI,EXCEPTION
    ...
    public String getDetailLabel() {
        if ("".equalsIgnoreCase(details[EXCEPTION]))
          return getRequestURI();
        else
          return new StringBuffer(getRequestURI()).append("\n").append(details[EXCEPTION]).toString();
    }
    ...
    private void setException() {
       if (requestException==null && request instanceof HttpServletRequest) {
        ...
        if (requestException!=null) {
          ...
          MonitorFactory.add(new MonKeyImp(label, getDetails(), "Exception"), 1);  
          MonitorFactory.add(new MonKeyImp(MonitorFactory.EXCEPTIONS_LABEL, getDetails(), "Exception"), 1);  
        }
    }
